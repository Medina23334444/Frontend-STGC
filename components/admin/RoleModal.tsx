...
export default function RoleModal({ isOpen, onClose, onSubmit, availablePermissions, initialData }: RoleModalProps) {
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors, isSubmitting } } = useForm<RoleCreateInput, unknown, RoleCreateOutput>({ 
  ...
  const handleFormSubmit = (formData: RoleCreateOutput) => {
    return onSubmit({
      name: formData.name,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4"> 
          ...
          <div>
            <label htmlFor="permission_ids" className="block text-sm font-medium text-gray-700 mb-2">Permisos Asignados</label>
            <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-md p-2 space-y-2 bg-gray-50">
              {availablePermissions.length === 0 ? (
                <p className="text-xs text-gray-500 text-center italic">No hay permisos disponibles.</p>
              ) : (
