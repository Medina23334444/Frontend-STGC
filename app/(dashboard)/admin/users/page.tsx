...
export default function UsersPage() {
  // Importamos toda la lógica desde el hook
  const {
    users,
    loading,
    isModalOpen,
                  ...
                  users.map((u) => (
                    <tr key={u.id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-4 text-center">
                        {/* ✨ Aplicamos la lógica visual de forma limpia ✨ */}
                        <span className={`inline-flex items-center justify-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-semibold border ${getStatusBadge(u.status)}`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${getStatusDot(u.status)}`}></span>
                          {u.status}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-center">
